      ...
      <head>
        ...
        <style>
          ...
          * { box-sizing: border-box; }
          ...
          .page { width: 190mm; min-height: 277mm; border: 2px solid #101010; padding: 8mm; margin: 0 auto 10mm; break-after: page; page-break-after: always; overflow: hidden; }
          ...
          @media print {
            ...
            .page { margin: 0; width: 190mm; min-height: 277mm; }
          }
        </style>
      </head>
      <body>${html}</body>
...
function buildVoucherHtml({
  ...
  const accountRateHtml = accountRate ? `<br/><small>سعر الصرف: ${accountRate}</small>` : "";
  const cashboxRateHtml = cashboxRate ? `<br/><small>سعر الصرف: ${cashboxRate}</small>` : "";
    ...
    <div class="page">
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td>${accountCode}</td>
            <td>${operation.account}${accountRateHtml}</td>
            <td>${cashboxCode} ${cashboxCurrency}${cashboxRateHtml}</td>
            <td>${operation.statement ?? "—"}</td>
            <td>${operation.amount.toLocaleString()} ${accountCurrency}</td>
          </tr>
        </tbody>
      </table>
    ...
    >
          ...
          return (
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  ...
                  <p className={`text-xs ${isDark ? "text-slate-400" : "text-slate-500"}`}>
                    {formatDateTimeNoSeconds(item.datetime)} | صندوق: #{box?.code ?? item.cashboxId} {meta.cashboxCurrency}
              ...
              <div className="mt-2 flex flex-wrap gap-2 text-sm font-bold text-cyan-300">
                <span>{item.amount.toLocaleString()} {meta.accountCurrency}</span>
              </div>
            </article>
          );
        })}
      ...
      {openForm ? (
        <div className="fixed inset-0 z-40 overflow-y-auto bg-slate-950/70 p-3 sm:p-4">
          <div className={`mx-auto my-4 max-h-[calc(100vh-2rem)] w-full max-w-2xl overflow-y-auto rounded-2xl border p-4 sm:p-5 ${isDark ? "border-slate-700 bg-slate-900 text-slate-100" : "border-slate-300 bg-white text-slate-900"}`}>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              ...
              <div className="md:col-span-2">
                <div className="grid grid-cols-[1fr_auto] gap-2">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (form.account && e.target.value !== form.account) setForm((f) => ({ ...f, account: "" }));
                    }}
                    placeholder={form.account ? form.account : "ابحث عن حساب"}
                ...
                <div className={`mt-2 max-h-36 overflow-y-auto rounded-xl border ${isDark ? "border-slate-700 bg-slate-800/70" : "border-slate-300 bg-slate-50"}`}>
                  {formAccountOptions.map((account) => (
                    <button
                      key={`pick-acc-${account.id}`}
                      type="button"
                      onClick={() => selectAccountForForm(account.name)}
                      className={`flex w-full items-center justify-between gap-3 px-3 py-2 text-right text-sm ${
                  ...
                  {formAccountOptions.length === 0 ? (
                    <div className={isDark ? "px-3 py-2 text-sm text-slate-400" : "px-3 py-2 text-sm text-slate-500"}>لا توجد حسابات مطابقة</div>
                  ) : null}
                </div>
              </div>
              <div className="grid grid-cols-[1fr_auto] gap-2">
  ...
  const printReport = () => {
    ...
    const rowsHtml = built
      .map((op) => {
        ...
        const accountRateHtml = meta.accountRate ? `<br/><small>سعر الصرف: ${meta.accountRate}</small>` : "";
        const cashboxRateHtml = meta.cashboxRate ? `<br/><small>سعر الصرف: ${meta.cashboxRate}</small>` : "";
        return `<tr>
          <td>${op.id}</td>
          <td>${parseDateOnly(op.datetime)}</td>
          <td>${op.type}</td>
          <td>${op.account}${accountRateHtml}</td>
          <td>${boxCode} ${meta.cashboxCurrency}${cashboxRateHtml}</td>
          <td>${debit ? `${debit.toLocaleString()} ${meta.accountCurrency}` : "0"}</td>
          <td>${credit ? `${credit.toLocaleString()} ${meta.accountCurrency}` : "0"}</td>
        </tr>`;
      })
      .join("");
              ...
              {rows.map((op) => {
                ...
                return (
                  <tr key={`op-${op.type}-${op.id}-${op.datetime}`} className="border-t border-slate-700/30">
                    ...
                    <td className="px-3 py-2">
                      <div>{op.account}</div>
                      {meta.accountRate ? <div className={isDark ? "text-xs text-slate-400" : "text-xs text-slate-500"}>سعر الصرف: {meta.accountRate}</div> : null}
                    </td>
                    <td className="px-3 py-2">
                      <div>{boxCode} {meta.cashboxCurrency}</div>
                      {meta.cashboxRate ? <div className={isDark ? "text-xs text-slate-400" : "text-xs text-slate-500"}>سعر الصرف: {meta.cashboxRate}</div> : null}
                    </td>
                    {reportMode === "تحليلي" ? <td className="px-3 py-2">{op.statement ?? "—"}</td> : null}
                    <td className="px-3 py-2 text-emerald-300">{op.type === "قبض" ? `${op.amount.toLocaleString()} ${meta.accountCurrency}` : "0"}</td>
                    <td className="px-3 py-2 text-rose-300">{op.type === "صرف" ? `${op.amount.toLocaleString()} ${meta.accountCurrency}` : "0"}</td>
                  </tr>
                );
              })}
            </tbody>
        ...
        .map((group, idx) => {
          const rows = group.entries
            .map((entry) => {
              ...
              const note = [
                entry.statement ?? "—",
                `${meta.cashboxCurrency}${meta.cashboxRate ? ` | سعر الصرف: ${meta.cashboxRate}` : ""}`,
              ].join("<br/>");
              return `<tr>
                    ...
                    {group.entries.map((entry) => {
                      ...
                      return (
                        <tr key={`ar-${group.account}-${entry.type}-${entry.id}-${entry.datetime}`} className="border-t border-slate-700/30">
                          ...
                          <td className="px-2 py-1 text-emerald-300">{entry.type === "قبض" ? `${entry.amount.toLocaleString()} ${meta.accountCurrency}` : "0"}</td>
                          <td className="px-2 py-1 text-rose-300">{entry.type === "صرف" ? `${entry.amount.toLocaleString()} ${meta.accountCurrency}` : "0"}</td>
                        </tr>
                      );
                    })}
                  </tbody>
...
export default function HomePage() {
  ...
  const buildNotifyMessage = (operation: OperationItem) => {
    ...
    const amountWords = numberToArabicWords(operation.amount);
    return `${head}\n${sideWord} مبلغ وقدره ${operation.amount.toLocaleString()}\nكتابة: ${amountWords}\nوذلك مقابل / ${operation.statement ?? "—"}`;
  };
