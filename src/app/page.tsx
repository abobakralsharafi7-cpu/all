...
type NativeToolsPlugin = {
  ...
  printHtml: (options: { title: string; html: string }) => Promise<{ status: "opened" }>;
      ...
      <head>
        ...
        <style>
          ...
          * { box-sizing: border-box; }
          ...
          body { font-family: Tahoma, Arial, sans-serif; width: 210mm; min-height: 297mm; }
          .page { width: 190mm; border: 2px solid #101010; padding: 6mm; margin: 0 auto; break-after: auto; page-break-after: auto; overflow: hidden; }
          ...
          .page-break + .page { break-before: page; page-break-before: always; }
          @media screen { body { padding: 12px; } }
          @media print {
            html, body { width: 210mm; min-height: 297mm; }
            body { padding: 0; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
            .page { margin: 0; width: 190mm; }
          }
        </style>
      </head>
      <body>${html}</body>
  ...
  return `
    <div class="page">
      ...
      <div class="amount-grid">
        ...
        <div class="amount-box">المبلغ (رقماً): ${formatAmount(cashboxAmount, cashboxCurrency)}</div>
      </div>
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td class="compact-cell">${accountCode}</td>
            <td>${operation.account}</td>
            <td class="compact-cell">${cashboxCode}${cashboxRateHtml}</td>
            <td class="statement-cell">${operation.statement ?? "—"}</td>
            <td>${formatAmount(accountAmount, accountCurrency)}</td>
          </tr>
        </tbody>
      </table>
...
function ModalShell({
  ...
  return (
    <div className="fixed inset-0 z-[80] overflow-y-auto bg-slate-950/70 p-3 sm:p-4">
      <div
  ...
  return (
    ...
    >
      <div className="my-auto grid gap-5">
        ...
        <div className={`text-center text-[11px] ${isDark ? "text-slate-400" : "text-slate-500"}`}>
          <p>طور بحب من قبل</p>
          <a
            href="https://www.facebook.com/ProfessorGold11/"
            target="_blank"
            rel="noreferrer"
            className={`mt-1 inline-flex items-center justify-center gap-2 rounded-full px-3 py-1 transition ${
              isDark ? "hover:bg-slate-800 hover:text-cyan-200" : "hover:bg-slate-100 hover:text-blue-700"
            }`}
          >
            <span aria-hidden="true" className="grid size-5 place-items-center rounded-full bg-blue-600 text-[10px] font-bold text-white">f</span>
            <span aria-hidden="true" className="grid size-5 place-items-center rounded-full bg-sky-500 text-[10px] font-bold text-white">✈</span>
            <span className="font-semibold">professorgold11</span>
          </a>
        </div>
      </div>
    </section>
...
function VoucherListScreen({
  ...
  return (
    ...
    >
      ...
      {openForm ? (
        <div className="fixed inset-0 z-40 overflow-y-auto bg-slate-950/70 p-3 sm:p-4">
          <div className={`mx-auto my-4 max-h-[calc(100vh-2rem)] w-full max-w-2xl overflow-y-auto overflow-x-hidden rounded-2xl border p-3 sm:p-5 ${isDark ? "border-slate-700 bg-slate-900 text-slate-100" : "border-slate-300 bg-white text-slate-900"}`}>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <div className="grid grid-cols-[minmax(128px,0.82fr)_minmax(96px,0.55fr)] gap-2 md:col-span-2">
...
function OperationsReportScreen({
  ...
  const printReport = async () => {
    ...
    if (reportMode === "تحليلي") {
      const pages = printRows
        .map((op, idx) => {
          ...
          return `${buildVoucherHtml({ operation: op, settings, accountCode: accCode, cashboxCode: boxCode, includeAttachment: Boolean(op.imageUrl) })}${idx < printRows.length - 1 ? '<div class="page-break"></div>' : ""}`;
        })
        .join("");

      const ok = await renderPrintWindow("تقارير العمليات - تحليلي", pages);
      onPrinted(ok);
      return;
    }
    ...
    const html = Array.from(byCurrency.entries())
      .map(([curr, currencyRows], pageIdx) => {
        const rowsHtml = currencyRows
          .map((op) => {
            ...
            const cashboxRateHtml = exchangeDisplay ? `<br/><small>سعر الصرف: ${formatNumber(exchangeDisplay)}</small>` : "";
            return `<tr>
              <td class="compact-cell">${op.id}</td>
              <td class="compact-cell">${parseDateOnly(op.datetime)}</td>
              <td class="compact-cell">${op.type}</td>
              <td>${op.account}</td>
              <td class="compact-cell">${boxCode}${cashboxRateHtml}</td>
              <td class="compact-cell">${curr}</td>
              <td>${debit ? formatNumber(debit) : "0"}</td>
              <td>${credit ? formatNumber(credit) : "0"}</td>
            </tr>`;
          })
          .join("");
        ...
        return `
          <div class="page">
            ${buildHeaderHtml(settings)}
            <div class="voucher-title"><div class="voucher-box">تقرير العمليات إجمالي</div><div class="meta">العملة: ${curr}</div></div>
            <table>
              <colgroup>
                <col style="width: 9%" />
                <col style="width: 11%" />
              ...
              <thead><tr><th>رقم السند</th><th>التاريخ</th><th>النوع</th><th>الحساب</th><th>الصندوق</th><th>العملة</th><th>مدين</th><th>دائن</th></tr></thead>
              <tbody>${rowsHtml}</tbody>
              <tfoot><tr><td colspan="6">الإجمالي</td><td>${formatNumber(totalDebit)}</td><td>${formatNumber(totalCredit)}</td></tr></tfoot>
            </table>
          </div>
          ${pageIdx < byCurrency.size - 1 ? '<div class="page-break"></div>' : ""}
        `;
      })
      .join("");

    const ok = await renderPrintWindow("تقارير العمليات - إجمالي", html);
    onPrinted(ok);
  };
...
function AccountsReportScreen({
  ...
  const printReport = async () => {
        ...
        .map((group, idx) => {
          const rows = group.entries
            .map((entry) => {
              ...
              return `<tr>
                <td class="compact-cell">${parseDateOnly(entry.datetime)}</td>
                <td class="compact-cell">${entry.type}</td>
                <td class="compact-cell">${entry.id}</td>
                <td class="statement-cell">${entry.statement ?? "—"}</td>
                <td>${debit ? formatNumber(debit) : "0"}</td>
                <td>${credit ? formatNumber(credit) : "0"}</td>
              </tr>`;
            })
            .join("");
          const openingRow = group.openingDebit || group.openingCredit ? `<tr><td></td><td></td><td></td><td class="statement-cell">رصيد افتتاحي</td><td>${group.openingDebit ? formatNumber(group.openingDebit) : "0"}</td><td>${group.openingCredit ? formatNumber(group.openingCredit) : "0"}</td></tr>` : "";
          ...
          return `
            <div class="page">
              ...
              <table>
                ...
                <tfoot><tr><td colspan="4">الإجمالي</td><td>${formatNumber(group.debit)}</td><td>${formatNumber(group.credit)}</td></tr></tfoot>
              </table>
              <div class="line">الرصيد: ${formatBalanceWithNature(group.balance, group.currency, group.nature)}</div>
            </div>
            ${idx < printAnalytical.length - 1 ? '<div class="page-break"></div>' : ""}
          `;
        })
        .join("");

      const ok = await renderPrintWindow("تقارير الحسابات - تحليلي", pages);
      onPrinted(ok);
      return;
    }
    ...
    const html = Array.from(aggregateByCurrency.entries())
      .map(([curr, currencyRows], idx) => {
        const rows = currencyRows
          .map((row) => `<tr><td>${row.account}</td><td class="compact-cell">${row.currency}</td><td>${formatNumber(row.debit)}</td><td>${formatNumber(row.credit)}</td><td>${formatBalanceWithNature(row.balance, row.currency, row.nature)}</td></tr>`)
          .join("");
        ...
        return `
          <div class="page">
            ${buildHeaderHtml(settings)}
            <div class="voucher-title"><div class="voucher-box">تقرير الحسابات إجمالي</div><div class="meta">العملة: ${curr}</div></div>
            <table>
              <thead><tr><th>الحساب</th><th>العملة</th><th>مدين</th><th>دائن</th><th>الرصيد</th></tr></thead>
              <tbody>${rows}</tbody>
              <tfoot><tr><td colspan="2">الإجمالي</td><td>${formatNumber(totalDebit)}</td><td>${formatNumber(totalCredit)}</td><td></td></tr></tfoot>
            </table>
          </div>
          ${idx < aggregateByCurrency.size - 1 ? '<div class="page-break"></div>' : ""}
        `;
      })
      .join("");

    const ok = await renderPrintWindow("تقارير الحسابات - إجمالي", html);
    onPrinted(ok);
  };
        ...
        const tableRows = group.entries
          .map((entry) => {
            ...
            return `<tr>
              ...
              <td class="compact-cell">${entry.type}</td>
              <td class="compact-cell">${entry.id}</td>
              <td class="compact-cell">${account?.accountCode ?? "—"}</td>
              <td>${entry.account}</td>
              <td class="statement-cell">${entry.statement ?? "—"}</td>
              <td>${debit ? formatNumber(debit) : "0"}</td>
              <td>${credit ? formatNumber(credit) : "0"}</td>
          ...
          .join("");
        const openingRow = group.openingDebit || group.openingCredit ? `<tr><td></td><td></td><td></td><td></td><td></td><td class="statement-cell">رصيد افتتاحي</td><td>${group.openingDebit ? formatNumber(group.openingDebit) : "0"}</td><td>${group.openingCredit ? formatNumber(group.openingCredit) : "0"}</td></tr>` : "";

        return `
          <div class="page">
            ${buildHeaderHtml(settings)}
            <div class="voucher-title"><div class="voucher-box">تقرير صندوق تحليلي</div><div class="meta">الصندوق: ${group.code} - ${group.name} (${group.currency})</div></div>
            <table>
              <colgroup>
                <col style="width: 10%" />
                <col style="width: 8%" />
              ...
              <thead><tr><th>التاريخ</th><th>النوع</th><th>رقم العملية</th><th>رقم الحساب</th><th>اسم الحساب</th><th>البيان</th><th>مدين</th><th>دائن</th></tr></thead>
              <tbody>${openingRow}${tableRows}</tbody>
              <tfoot><tr><td colspan="6">الإجمالي</td><td>${formatNumber(group.debit)}</td><td>${formatNumber(group.credit)}</td></tr></tfoot>
            </table>
            <div class="line">الرصيد: ${formatBalanceWithNature(group.balance, group.currency, group.nature)}</div>
          </div>
          ${idx < printRows.length - 1 ? '<div class="page-break"></div>' : ""}
        `;
      })
      .join("");

    const ok = await renderPrintWindow("تقارير الصناديق", pages);
    onPrinted(ok);
  };
...
export default function HomePage() {
  ...
  const backupNow = async () => {
    ...
    try {
      const result = await downloadJsonFile(`proof-daftar-backup-${Date.now()}.json`, payload);
      addToast(saveResultMessage(result, "النسخة الاحتياطية"), result === "cancelled" ? "error" : "success");
    } catch {
      addToast("تعذر حفظ النسخة الاحتياطية", "error");
    }
  };
