        ...
        <div className="fixed inset-0 z-40 overflow-y-auto bg-slate-950/70 p-3 sm:p-4">
          <div className={`mx-auto my-4 max-h-[calc(100vh-2rem)] w-full max-w-2xl overflow-y-auto overflow-x-hidden rounded-2xl border p-3 sm:p-5 ${isDark ? "border-slate-700 bg-slate-900 text-slate-100" : "border-slate-300 bg-white text-slate-900"}`}>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <div className="grid grid-cols-[minmax(150px,1fr)_minmax(92px,0.7fr)] gap-2 md:col-span-2">
...
function AccountsReportScreen({
  ...
  const build = () => {
    ...
    const analytical = Array.from(grouped.entries()).map(([key, entries]) => {
      ...
      return { account, currency: curr, entries, openingDebit, openingCredit, debit: totalDebit, credit: totalCredit, balance, nature: balance >= 0 ? "لكم" : "عليكم" };
        ...
        .map((group, idx) => {
          ...
          return `
            <div class="page">
              ...
              <div class="line">الرصيد: ${formatNumber(group.balance)} ${group.currency} (${group.nature})</div>
            </div>
    ...
    const html = Array.from(aggregateByCurrency.entries())
      .map(([curr, currencyRows], idx) => {
        const rows = currencyRows
          .map((row) => `<tr><td>${row.account}</td><td class="compact-cell">${row.currency}</td><td>${formatNumber(row.debit)}</td><td>${formatNumber(row.credit)}</td><td>${formatNumber(row.balance)} ${row.currency} (${row.nature})</td></tr>`)
          .join("");
        ...
        <div className="space-y-4">
          {analyticalRows.map((group) => (
            <article key={`ana-${group.account}-${group.currency}`} className={`rounded-xl border p-3 ${isDark ? "border-slate-700 bg-slate-800/70" : "border-slate-300 bg-slate-50"}`}>
              ...
              <p className="mt-2 text-sm text-cyan-300">الرصيد: {formatNumber(group.balance)} {group.currency} ({group.nature})</p>
            </article>
          ))}
        </div>
      ) : null}
            ...
            <tbody>
              {aggregateRows.map((row) => (
                <tr key={`agg-${row.account}-${row.currency}`} className="border-t border-slate-700/30">
                  ...
                  <td className="px-3 py-2 text-cyan-300">{formatNumber(row.balance)} ({row.nature})</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
...
export default function HomePage() {
  ...
  const restoreFromFile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      addToast("تمت استعادة النسخة الاحتياطية، سيتم إعادة تشغيل التطبيق لعرض البيانات", "success");
