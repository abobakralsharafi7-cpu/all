...
/* ==== ترقيم صفحات الطباعة (A4) ==== */
const PRINT_ROWS_FIRST_PAGE = 18;
const PRINT_ROWS_NEXT_PAGE = 24;
...
type StoredOperation = OperationItem & { __enc?: EncryptedPayload };
...
async function encryptOperationRecord(operation: OperationItem): Promise<StoredOperation> {
  ...
  try {
    ...
    const payload = await encryptWithPassphrase(JSON.stringify(secret), getDeviceKey());
    const shell: StoredOperation = { ...operation, __enc: payload };
    SENSITIVE_FIELDS.forEach((field) => {
...
async function decryptOperationRecord(record: StoredOperation): Promise<OperationItem> {
  ...
  try {
    const json = await decryptWithPassphrase(record.__enc, getDeviceKey());
    const secret = JSON.parse(json) as Partial<OperationItem>;
...
/* =========================================================================
   ...
   ========================================================================= */
const IMAGE_TARGET_BYTES = 90 * 1024; // الحجم المستهدف لكل مرفق
const IMAGE_MAX_BYTES = 160 * 1024; // الحد الأقصى المطلق
const IMAGE_MAX_WIDTH = 1240; // يكفي لقراءة نص مستند A4 مصوّر
...
async function compressImageFile(file: File, maxWidth = IMAGE_MAX_WIDTH, quality = 0.72): Promise<string> {
  ...
  // تخفيض الجودة تدريجيًا مع الحفاظ على الوضوح
  const qualitySteps = [0.62, 0.54, 0.48, 0.42];
  for (const step of qualitySteps) {
    const candidate = renderAt(maxWidth, step);
    if (!candidate) break;
    best = candidate;
    if (dataUrlBytes(candidate) <= IMAGE_TARGET_BYTES) return candidate;
  }

  // إن بقيت كبيرة، نقلّل الأبعاد تدريجيًا (لا نقل عن 900px لضمان قراءة النص)
  const widthSteps = [1100, 1000, 900];
  for (const stepWidth of widthSteps) {
    const candidate = renderAt(stepWidth, 0.5);
    if (!candidate) break;
    best = candidate;
    if (dataUrlBytes(candidate) <= IMAGE_MAX_BYTES) return candidate;
  }
...
function chunkPrintRows(rows: string[]) {
  if (rows.length === 0) return [[]] as string[][];
  const pages: string[][] = [];
  let index = 0;
  while (index < rows.length) {
    const size = pages.length === 0 ? PRINT_ROWS_FIRST_PAGE : PRINT_ROWS_NEXT_PAGE;
...
function buildPaginatedReport(blocks: PrintPageBlock[], settings: AppSettings) {
  type RenderedPage = { inner: string };
  const rendered: RenderedPage[] = [];

  blocks.forEach((block) => {
    const chunks = chunkPrintRows(block.rowsHtml);
  ...
  return rendered
    .map((page, index) => {
      ...
      return `<div class="page">${page.inner}${counter}</div>${index < rendered.length - 1 ? '<div class="page-break"></div>' : ""}`;
    })
    .join("");
}
      ...
      <head>
        ...
        <style>
          ...
          * { box-sizing: border-box; }
          ...
          .page { position: relative; width: 190mm; min-height: 277mm; border: 2px solid #101010; padding: 6mm 6mm 12mm 6mm; margin: 0 auto; break-after: auto; page-break-after: auto; overflow: hidden; }
          .page-counter { position: absolute; bottom: 4mm; right: 6mm; font-size: 9px; color: #334155; direction: ltr; letter-spacing: 0.5px; }
        ...
        <div className="fixed inset-0 z-40 overflow-y-auto bg-slate-950/70 p-3 sm:p-4">
          <div className={`mx-auto my-4 max-h-[calc(100vh-2rem)] w-full max-w-2xl overflow-y-auto overflow-x-hidden rounded-2xl border p-3 sm:p-5 ${isDark ? "border-slate-700 bg-slate-900 text-slate-100" : "border-slate-300 bg-white text-slate-900"}`}>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <div className="grid grid-cols-[minmax(108px,0.62fr)_minmax(96px,0.55fr)] gap-2 md:col-span-2">
                <input type="datetime-local" value={form.datetimeInput} onChange={(e) => setForm((f) => ({ ...f, datetimeInput: e.target.value }))} className={`${inputClass} px-2 text-xs`} />
...
export default function HomePage() {
  ...
  const backupNow = async (passphrase?: string) => {
    ...
    try {
      ...
      if (passphrase && passphrase.length > 0 && hasWebCrypto()) {
        ...
        fileText = JSON.stringify({ proofDaftar: true, appVersion: APP_VERSION, encrypted }, null, 2);
        filename = `proof-daftar-backup-${Date.now()}.pdbk.json`;
      }

      let result: SaveResult;
      if (isAndroidApp()) {
        const native = await NativeTools.saveTextFile({ filename, mimeType: "application/json", text: fileText });
        result = native.status;
      } else {
        const blob = new Blob([fileText], { type: "application/json" });
        result = await saveBlobToDevice(blob, filename, "ملف نسخة احتياطية", { "application/json": [".json"] });
      }
  ...
  const restoreFromFile = async (event: ChangeEvent<HTMLInputElement>, passphrase?: string) => {
    const file = event.target.files?.[0];
    if (!file) return;
        ...
        {selectedScreen === "system-backup" ? (
          <section className={`space-y-4 rounded-3xl border p-4 sm:p-6 ${isDark ? "border-slate-700/70 bg-slate-900/60 text-slate-100" : "border-slate-300 bg-white/95 text-slate-900"}`}>
            ...
            <div className={`grid gap-2 rounded-2xl border p-4 text-sm sm:grid-cols-2 ${isDark ? "border-slate-700 bg-slate-800/60" : "border-slate-300 bg-slate-50"}`}>
              <p>🔐 تشفير قاعدة البيانات: <span className="font-bold text-emerald-400">{hasWebCrypto() ? "مفعّل (AES-256-GCM)" : "غير متاح"}</span></p>
              <p>🛡️ التخزين الدائم: <span className={`font-bold ${persistentGranted ? "text-emerald-400" : "text-amber-400"}`}>{persistentGranted ? "مفعّل — محمي من تطبيقات التنظيف" : "غير مؤكد"}</span></p>
              ...
              <p className="sm:col-span-2 opacity-70">إصدار التطبيق: {APP_VERSION} (بناء {APP_BUILD}) — تطوير: {APP_DEVELOPER}</p>
            </div>
            ...
            <div className={`space-y-2 rounded-2xl border p-4 ${isDark ? "border-slate-700 bg-slate-800/40" : "border-slate-300 bg-white"}`}>
              ...
              />
              <p className={`text-xs ${isDark ? "text-slate-400" : "text-slate-500"}`}>
                عند إدخال كلمة سر، يُشفَّر ملف النسخة بالكامل (AES-256-GCM) ولا يمكن لأحد الاطلاع عليه أو استعادته بدونها.
                استخدم نفس كلمة السر عند الاستعادة. ⚠️ نسيان كلمة السر يعني عدم القدرة على استرجاع النسخة نهائيًا.
              </p>
            </div>

            <div className="flex flex-wrap gap-2">
              <button onClick={() => { void backupNow(backupPassphrase || undefined); }} className="rounded-xl bg-emerald-500 px-4 py-2 text-sm font-bold text-emerald-950">
                {backupPassphrase ? "نسخ احتياطي مشفّر الآن" : "نسخ احتياطي الآن"}
              </button>
              ...
              <input ref={restoreInputRef} type="file" accept="application/json" className="hidden" onChange={(event) => { void restoreFromFile(event, backupPassphrase || undefined); }} />
            </div>
          </section>
        ) : null}
      </div>
