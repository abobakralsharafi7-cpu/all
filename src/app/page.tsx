...
export default function HomePage() {
  ...
  const backupNow = async (passphrase?: string) => {
    ...
    try {
      ...
      /* النسخة الاحتياطية تتضمن الصور الفعلية (لا المراجع) حتى تعمل
         الاستعادة على أي جهاز، وتبقى متوافقة مع النسخ القديمة. */
      const allOps = [...payload.paymentOperations, ...payload.receiptOperations];
      if (allOps.some((op) => isImageRef(op.imageUrl))) {
        setPrintProgress(true, "جارٍ تجميع الصور المرفقة...", 35);
        const imageMap = await loadImages(allOps.map((op) => op.imageUrl));
        const inline = (op: OperationItem) =>
          isImageRef(op.imageUrl) ? { ...op, imageUrl: imageMap.get(op.imageUrl as string) } : op;
        payload.paymentOperations = payload.paymentOperations.map(inline);
        payload.receiptOperations = payload.receiptOperations.map(inline);
      }
