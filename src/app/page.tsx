...
const PRINT_PAGE_UNITS = 268; // السعة الكلية بوحدات (1 وحدة ≈ 1mm)
const PRINT_HEADER_UNITS = 46; // ترويسة الشركة + عنوان التقرير
const PRINT_THEAD_UNITS = 9; // رأس الجدول (يتكرر بكل صفحة)
const PRINT_FOOT_UNITS = 10; // صف الإجمالي
const PRINT_TAIL_UNITS = 12; // سطر الرصيد
const PRINT_ROW_UNITS = 8.2; // ارتفاع الصف ذي السطر الواحد
const PRINT_ROW_EXTRA_LINE_UNITS = 4.6; // لكل سطر إضافي في خانة البيان
...
function estimateRowUnits(rowHtml: string) {
  const statementMatch = rowHtml.match(/class="statement-cell"[^>]*>([\s\S]*?)<\/td>/);
  const statement = statementMatch ? statementMatch[1].replace(/<[^>]+>/g, "") : "";
  // خانة البيان تتسع لنحو 55 حرفًا في السطر الواحد
  const lines = Math.max(1, Math.ceil(statement.length / 55), (statement.match(/\n/g)?.length ?? 0) + 1);
  const extraFromBr = (rowHtml.match(/<br\s*\/?>/gi)?.length ?? 0) * PRINT_ROW_EXTRA_LINE_UNITS;
  return PRINT_ROW_UNITS + (lines - 1) * PRINT_ROW_EXTRA_LINE_UNITS + extraFromBr;
}
...
function chunkPrintRows(rows: string[], leadingUnits: number, footUnits: number, tailUnits: number) {
  ...
  /* موازنة أخيرة: إذا بقي صف أو صفان وحيدان في الصفحة الأخيرة،
     نُوزّع بعض الصفوف من الصفحة السابقة لتفادي صفحة شبه فارغة */
  if (pages.length >= 2) {
    const last = pages[pages.length - 1];
    const prev = pages[pages.length - 2];
    const lastUnits = PRINT_HEADER_UNITS + PRINT_THEAD_UNITS + footUnits + tailUnits
      + last.reduce((sum, row) => sum + estimateRowUnits(row), 0);

    if (last.length <= 2 && lastUnits < PRINT_PAGE_UNITS * 0.45) {
      const moveCount = Math.min(Math.floor(prev.length / 3), prev.length - 1);
      if (moveCount > 0) {
        const moved = prev.splice(prev.length - moveCount, moveCount);
      ...
      <head>
        ...
        <style>
          ...
          * { box-sizing: border-box; }
          ...
          /* ارتفاع الصفحة مضبوط بدقة على A4 (297mm - 20mm هوامش = 277mm)
             box-sizing: border-box ضروري كي لا يدفع الإطار والحشو المحتوى لصفحة إضافية فارغة */
          .page { position: relative; box-sizing: border-box; width: 190mm; height: 277mm; border: 2px solid #101010; padding: 5mm 5mm 10mm 5mm; margin: 0 auto; overflow: hidden; break-inside: avoid; page-break-inside: avoid; }
          .page-counter { position: absolute; bottom: 3mm; right: 5mm; font-size: 9px; color: #334155; direction: ltr; letter-spacing: 0.5px; }
          ...
          @media print {
            html, body { width: 210mm; height: auto; min-height: 0; margin: 0; padding: 0; }
            body { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
            .page { margin: 0; width: 190mm; height: 277mm; }
            /* آخر صفحة يجب ألا تُتبع بفاصل — يمنع الصفحة الفارغة الأخيرة */
...
export default function HomePage() {
  ...
  const backupNow = async (passphrase?: string) => {
    ...
    try {
      ...
      // الحفظ عبر Blob دائمًا: تمرير عدة ميجابايت كنص عبر جسر Capacitor
      // كان يسبب إغلاق التطبيق وإنشاء ملف بحجم صفر
      const blob = new Blob([fileText], { type: "application/json" });
      const result = await saveBlobToDevice(blob, filename, "ملف نسخة احتياطية", {
