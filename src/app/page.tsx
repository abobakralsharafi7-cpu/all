...
type NativeToolsPlugin = {
  ...
  clearWorkCache: () => Promise<{ status: string }>;
...
/* مخزن الصور المنفصل: الصور لا تُحمَّل مع السندات بل عند العرض فقط.
   هذا يقلّل استهلاك الذاكرة بنحو 99% مع آلاف السندات المصوّرة. */
const IMAGE_STORE = "operationImages";
...
/* ==== حد آمن لطباعة المرفقات ====
   كل مرفق يضيف ~61 كيلو حرف إلى نص التقرير. تجاوز هذا الحد يقترب من
   الحد الأقصى لسلسلة نصية في JavaScript ويسبب انهيارًا. */
const MAX_ATTACHMENTS_PER_PRINT = 1200;

/* ==== حد آمن لتشفير النسخة الاحتياطية ====
   التشفير يحتاج النص كاملًا في الذاكرة. فوق ~150 مليون حرف يقترب
   من حد سلسلة JavaScript فينهار التطبيق. النسخة غير المشفّرة
   تُكتب على أجزاء ولا يوجد لها حد. */
const MAX_ENCRYPTED_BACKUP_CHARS = 150 * 1024 * 1024;

/* ==== الإقلاع السريع ====
   فوق هذا العدد نعرض أحدث دفعة فورًا ثم نُكمل التحميل في الخلفية. */
const FAST_START_THRESHOLD = 400;
const FAST_START_PAGE = 150;
...
async function openLocalDb() {
  ...
  return new Promise<IDBDatabase>((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      // مخزن الصور المنفصل (يُنشأ تلقائيًا عند الترقية من الإصدار 2)
...
/* =========================================================================
   مخزن الصور المنفصل
   السند يحتفظ بمرجع "img:<id>" فقط، والصورة تُقرأ عند الحاجة.
   ========================================================================= */
function isImageRef(value?: string) {
  return typeof value === "string" && value.startsWith(IMAGE_REF_PREFIX);
}

/* ذاكرة مؤقتة صغيرة: تمنع إعادة قراءة نفس الصورة مرارًا أثناء التصفح */
const imageCache = new Map<string, string>();
const IMAGE_CACHE_LIMIT = 40;

function cacheImage(ref: string, dataUrl: string) {
...
async function putImageBlob(ref: string, dataUrl: string) {
  await withStore(IMAGE_STORE, "readwrite", async (store) => {
    await requestToPromise(store.put(dataUrl, ref));
  });
  cacheImage(ref, dataUrl);
}

/** يقرأ صورة واحدة. يرجع المدخل كما هو إن لم يكن مرجعًا (توافق مع القديم). */
async function loadImage(ref?: string): Promise<string | undefined> {
  if (!ref) return undefined;
  if (!isImageRef(ref)) return ref; // صورة قديمة مضمّنة مباشرة
  const cached = imageCache.get(ref);
  if (cached) return cached;
  try {
    const value = await getStoreValue<string>(IMAGE_STORE, ref);
    if (value) cacheImage(ref, value);
    return value;
  } catch {
    return undefined;
  }
...
/** يقرأ عدة صور دفعة واحدة (للطباعة والتقارير). */
async function loadImages(refs: Array<string | undefined>): Promise<Map<string, string>> {
  const result = new Map<string, string>();
  const needed = Array.from(new Set(refs.filter((r): r is string => isImageRef(r))));
  if (needed.length === 0) return result;

  const BATCH = 60;
  for (let i = 0; i < needed.length; i += BATCH) {
    const slice = needed.slice(i, i + BATCH);
    await withStore(IMAGE_STORE, "readonly", async (store) => {
      const values = await Promise.all(slice.map((ref) => requestToPromise<string | undefined>(store.get(ref))));
      slice.forEach((ref, index) => {
        const value = values[index];
        if (value) result.set(ref, value);
      });
    });
    if (i + BATCH < needed.length) await new Promise((resolve) => setTimeout(resolve, 0));
  }
  return result;
}

/** يُرجع نسخًا من السندات بصورها الفعلية (للطباعة والعرض). */
async function withResolvedImages(operations: OperationItem[]): Promise<OperationItem[]> {
  const map = await loadImages(operations.map((op) => op.imageUrl));
  return operations.map((op) =>
    isImageRef(op.imageUrl) ? { ...op, imageUrl: map.get(op.imageUrl as string) } : op,
  );
}

async function deleteImage(ref?: string) {
  if (!isImageRef(ref)) return;
  imageCache.delete(ref as string);
  try {
    await withStore(IMAGE_STORE, "readwrite", async (store) => {
      await requestToPromise(store.delete(ref as string));
    });
  } catch {
    /* لا يمنع حذف السند */
  }
...
/** يحوّل صورة مضمّنة إلى مرجع مخزَّن، ويعيد السند بالمرجع. */
async function externalizeImage(operation: OperationItem): Promise<OperationItem> {
  const url = operation.imageUrl;
  if (!url || isImageRef(url)) return operation;
  const ref = `${IMAGE_REF_PREFIX}${operation.type === "صرف" ? "p" : "r"}${operation.id}_${Date.now()}`;
  await putImageBlob(ref, url);
  return { ...operation, imageUrl: ref };
}
...
/** يضيف عنصرًا إلى مجموعة داخل Map بلا نسخ المصفوفة (يمنع بطء O(n²)). */
function pushToGroup<K, V>(map: Map<K, V[]>, key: K, value: V) {
...
/* =========================================================================
   القراءة بالمؤشر (Cursor) — أساس التحميل التدريجي
   المؤشر يقفز مباشرة إلى الموضع المطلوب عبر فهرس datetime،
   فسرعته ثابتة مهما كبر عدد السجلات (بخلاف OFFSET في SQL الذي يبطؤ).
   ========================================================================= */
async function readOperationsPage(
  storeName: string,
  limit: number,
  cursorKey?: IDBValidKey,
): Promise<{ rows: OperationItem[]; nextKey?: IDBValidKey; done: boolean }> {
  const raw: StoredOperation[] = [];
  ...
  await withStore(storeName, "readonly", (store) => {
    ...
    const range = cursorKey !== undefined ? IDBKeyRange.upperBound(cursorKey, true) : undefined;
    const request = index.openCursor(range ?? null, "prev");
    ...
    return new Promise<void>((resolve, reject) => {
      request.onsuccess = () => {
        ...
        raw.push(cursor.value as StoredOperation);
        lastKey = cursor.key;
        cursor.continue();
      };
      request.onerror = () => reject(request.error);
  ...
  const rows = await Promise.all(raw.map((record) => decryptOperationRecord(record)));
  return { rows, nextKey: lastKey, done: raw.length < limit };
}

/** عدد السجلات بلا قراءة محتواها (سريع جدًا). */
async function countOperations(storeName: string): Promise<number> {
  try {
    return await withStore<number>(storeName, "readonly", (store) => requestToPromise(store.count()));
  } catch {
    return 0;
  }
}
...
/* =========================================================================
   ترحيل الصور القديمة إلى المخزن المنفصل
   يعمل تلقائيًا وبصمت عند أول فتح بعد التحديث.
   آمن تمامًا: لا يُعدَّل السند إلا بعد نجاح كتابة الصورة،
   وإن فشل شيء تبقى الصورة مضمّنة كما هي (يعمل التطبيق طبيعيًا).
   ========================================================================= */
const IMAGE_MIGRATION_KEY = "imagesExternalized";
...
async function migrateImagesToStore(
  ...
  for (const storeName of [PAYMENT_STORE, RECEIPT_STORE]) {
    let records: StoredOperation[] = [];
    try {
      records = await getAllStoreRecords<StoredOperation>(storeName);
    } catch {
      continue;
    }

    // السجلات المشفّرة تُفك أولًا لمعرفة إن كانت تحمل صورة مضمّنة
    ...
    for (const record of records) {
      index += 1;
      try {
        const decoded = await decryptOperationRecord(record);
        const url = decoded.imageUrl;
        if (!url || isImageRef(url)) continue;

        // 1) اكتب الصورة في المخزن الجديد أولًا
        const ref = `${IMAGE_REF_PREFIX}${storeName === PAYMENT_STORE ? "p" : "r"}${decoded.id}_${Date.now()}`;
        await putImageBlob(ref, url);

        // 2) لا نحدّث السند إلا بعد التأكد من نجاح الكتابة
        const stored = await getStoreValue<string>(IMAGE_STORE, ref);
        if (!stored) {
          failed += 1;
          continue;
        }

        await putOperationRecord(storeName, { ...decoded, imageUrl: ref });
        migrated += 1;
      } catch {
        failed += 1; // الصورة تبقى مضمّنة — التطبيق يعمل طبيعيًا
      }
...
/* =========================================================================
   القياس الفعلي لارتفاع الصفوف (بدل التخمين)
   يُبنى جدول مطابق تمامًا لجدول الطباعة داخل عنصر مخفي خارج الشاشة،
   ثم يُقاس ارتفاع كل صف بالبكسل الحقيقي. هذا يعالج جذريًا اختلاف
   عرض الأعمدة بين التقارير واختلاف أطوال النصوص العربية.
   ========================================================================= */
const PX_PER_MM = 96 / 25.4;
/* العرض الداخلي الفعلي للجدول داخل .page:
   190mm (عرض الصفحة) − 4px (إطار 2px يمين ويسار) − 8mm (حشو 4mm يمين ويسار)
   القياس بعرض خاطئ (أوسع) يُقدّر أسطرًا أقل فيفيض المحتوى لصفحة إضافية. */
const A4_CONTENT_WIDTH_PX = 190 * PX_PER_MM - 4 - 8 * PX_PER_MM;
...
function measureRowHeights(
  ...
  footerHtml?: string,
  tailHtml?: string,
): { rows: number[]; leading: number; thead: number; foot: number; tail: number } | null {
  ...
  // نفس أنماط جدول الطباعة بالضبط
  host.innerHTML = `
    <style>
      .m-tbl { width:100%; border-collapse:collapse; table-layout:fixed;
               font-family:Tahoma,Arial,sans-serif; direction:rtl; }
      .m-tbl th, .m-tbl td { border:2px solid #111; padding:5px 6px; text-align:center;
               vertical-align:middle; font-size:13px; word-break:normal; overflow-wrap:break-word; }
      .m-tbl td.compact-cell, .m-tbl th.compact-cell { white-space:nowrap; }
      .m-tbl td.date-cell, .m-tbl th.date-cell { white-space:nowrap; font-size:12px;
               letter-spacing:-0.2px; padding-left:2px; padding-right:2px; direction:ltr; text-align:center; }
      .m-tbl td.name-cell, .m-tbl th.name-cell { text-align:right; white-space:normal;
               overflow-wrap:break-word; word-break:break-word; max-width:0; font-size:12px;
               padding-left:4px; padding-right:4px; }
      .m-tbl td.statement-cell, .m-tbl th.statement-cell { text-align:right; white-space:pre-wrap;
               overflow-wrap:anywhere; word-break:break-word; max-width:0; }
      .m-tbl td.amount-cell, .m-tbl th.amount-cell { white-space:nowrap; font-size:12px;
               letter-spacing:-0.2px; padding-left:3px; padding-right:3px; direction:ltr;
               text-align:center; overflow-wrap:normal; word-break:keep-all; }
      .m-tbl small { font-size:11px; }
      .m-line { font-family:Tahoma,Arial,sans-serif; direction:rtl; }
      .m-line .line { border:1px solid #222; border-radius:8px; padding:7px; margin-top:8px; font-size:17px; }
    </style>
    <table class="m-tbl">${colgroupHtml}${theadHtml}<tbody>${leadingRowsHtml ?? ""}${rowsHtml.join("")}</tbody>${footerHtml ? `<tfoot id="m-foot">${footerHtml}</tfoot>` : ""}</table>
    ${tailHtml ? `<div id="m-tail" class="m-line">${tailHtml}</div>` : ""}
  `;
  ...
  try {
    ...
    // إجبار المتصفح على إتمام التخطيط قبل القياس
    void (host as HTMLElement).offsetHeight;

    const thead = table.querySelector("thead");
    const theadPx = thead ? (thead as HTMLElement).getBoundingClientRect().height : 0;

    const bodyRows = Array.from(table.querySelectorAll("tbody > tr")) as HTMLElement[];
    const heights = bodyRows.map((tr) => tr.getBoundingClientRect().height);

    const hasLeading = Boolean(leadingRowsHtml);
    const leadingPx = hasLeading && heights.length > 0 ? heights[0] : 0;
    const rowPx = hasLeading ? heights.slice(1) : heights;

    if (rowPx.length !== rowsHtml.length) return null;

    const footEl = host.querySelector("#m-foot") as HTMLElement | null;
    const tailEl = host.querySelector("#m-tail") as HTMLElement | null;

    return {
      rows: rowPx.map((px) => px / PX_PER_MM),
      leading: leadingPx / PX_PER_MM,
      thead: theadPx / PX_PER_MM,
      foot: footEl ? footEl.getBoundingClientRect().height / PX_PER_MM : 0,
      tail: tailEl ? tailEl.getBoundingClientRect().height / PX_PER_MM : 0,
    };
  } catch {
    return null;
  } finally {
...
function chunkPrintRows(
  rows: string[],
  leadingUnits: number,
  footUnits: number,
  tailUnits: number,
  rowHeights?: number[],
  theadUnits: number = PRINT_THEAD_UNITS,
) {
  if (rows.length === 0) return [[]] as string[][];

  const heights = rowHeights ?? rows.map(estimateRowUnits);
  const pages: string[][] = [];
  let current: string[] = [];
  let used = PRINT_HEADER_UNITS + theadUnits + leadingUnits;
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    if (current.length > 0 && used + rowHeight + reserve > PRINT_PAGE_UNITS) {
      pages.push(current);
      current = [];
      used = PRINT_HEADER_UNITS + theadUnits;
    }
  ...
  /* موازنة الصفحة الأخيرة إن بقيت شبه فارغة */
  const heightOf = (row: string) => {
    const index = rows.indexOf(row);
    return index >= 0 ? heights[index] : estimateRowUnits(row);
  };
  const pageUnits = (rowsOfPage: string[], isLast: boolean) =>
    PRINT_HEADER_UNITS + theadUnits + (isLast ? footUnits + tailUnits : 0)
    + rowsOfPage.reduce((sum, row) => sum + heightOf(row), 0);
  ...
  if (pages.length >= 2) {
    ...
    for (let guard = 0; guard < 60; guard += 1) {
      ...
      const lastUnits = pageUnits(last, true);
      if (lastUnits >= PRINT_PAGE_UNITS * 0.62) break;

      const candidate = prev[prev.length - 1];
      const candidateUnits = heightOf(candidate);
...
function buildPaginatedReport(blocks: PrintPageBlock[], settings: AppSettings) {
  ...
  blocks.forEach((block) => {
    ...
    // القياس الفعلي أولًا؛ التقدير التخميني احتياط فقط
    const measured = measureRowHeights(
      block.rowsHtml,
      block.colgroupHtml,
      block.theadHtml,
      block.leadingRowsHtml,
      block.footerHtml,
      block.tailHtml,
    );

    const leadingUnits = measured
      ? measured.leading
      : block.leadingRowsHtml
        ? estimateRowUnits(block.leadingRowsHtml)
        : 0;
    const theadUnits = measured ? measured.thead : PRINT_THEAD_UNITS;
    // القياس الفعلي للإجمالي وسطر الرصيد بدل التقدير الثابت
    const realFootUnits = measured ? measured.foot : footUnits;
    const realTailUnits = measured ? measured.tail : tailUnits;

    const chunks = chunkPrintRows(
      block.rowsHtml,
      leadingUnits,
      realFootUnits,
      realTailUnits,
...
async function renderPrintWindow(title: string, html: string) {
  setPrintProgress(true, "جارٍ تجهيز التقرير...", 15);
      ...
      <head>
        ...
        <style>
          ...
          * { box-sizing: border-box; }
          ...
          .page { position: relative; box-sizing: border-box; display: flex; flex-direction: column; width: 100%; max-width: 190mm; min-height: 262mm; border: 2px solid #101010; padding: 4mm; margin: 0 auto; overflow: hidden; page-break-inside: avoid; break-inside: avoid; page-break-after: always; break-after: page; }
          .page > .page-counter { margin-top: auto; padding-top: 3mm; }
          .page:last-of-type { page-break-after: auto; break-after: auto; }
          /* العدّاد ضمن تدفّق الصفحة لا مطلقًا: العنصر المطلق داخل صفحة
             يتجاوز ارتفاعها يُدفَع إلى الورقة التالية فيختفي أو يُزاح ترقيمه. */
          .page-counter { margin-top: 3mm; text-align: right; font-size: 9px; color: #334155; direction: ltr; letter-spacing: 0.5px; page-break-inside: avoid; break-inside: avoid; }
          ...
          /* عمود التاريخ: لا يخرج النص أبدًا — مقاس ليتّسع لـ YYYY-MM-DD كاملًا */
          td.date-cell, th.date-cell { white-space: nowrap; vertical-align: middle; font-size: 12px;
                                       letter-spacing: -0.2px; padding-left: 2px; padding-right: 2px;
                                       direction: ltr; text-align: center; }
          /* عمود اسم الحساب: عرض ثابت، والأسماء الطويلة تلتف رأسيًا ولا تخرج */
          td.name-cell, th.name-cell { text-align: right; white-space: normal;
                                       overflow-wrap: break-word; word-break: break-word;
                                       max-width: 0; font-size: 12px; vertical-align: middle;
                                       padding-left: 4px; padding-right: 4px; }
          /* عمود البيان: يتوسّع رأسيًا فقط، وعرضه ثابت مهما طال النص */
          td.statement-cell, th.statement-cell { text-align: right; white-space: pre-wrap;
                                                 overflow-wrap: anywhere; word-break: break-word;
                                                 max-width: 0; }
          /* أعمدة المبالغ: تظهر كاملة على سطر واحد ولا تنكسر للأسفل */
          td.amount-cell, th.amount-cell { white-space: nowrap; vertical-align: middle;
  ...
  if (isAndroidApp()) {
    try {
      ...
      if (documentHtml.length > PRINT_BRIDGE_LIMIT) {
        // دفعات أكبر (2 ميجا) = عدد استدعاءات أقل بـ4 أضعاف عبر الجسر.
        // الجسر يتحمّلها بسهولة، والبطء كان من كثرة الاستدعاءات لا من حجمها.
      ...
      try {
        setPrintProgress(true, "جارٍ المحاولة بطريقة بديلة...", 70);
...
/* =========================================================================
   ...
   ========================================================================= */
type PrintProgress = { active: boolean; message: string; percent: number };
let printProgressListener: ((state: PrintProgress) => void) | null = null;

function setPrintProgress(active: boolean, message = "", percent = 0) {
  printProgressListener?.({ active, message, percent: Math.max(0, Math.min(100, Math.round(percent))) });
}

function PrintOverlay() {
  const [state, setState] = useState<PrintProgress>({ active: false, message: "", percent: 0 });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setState({ active: false, message: "", percent: 0 }), 45000);
  ...
  return (
    <div className="fixed inset-0 z-[95] flex items-center justify-center bg-slate-950/80 p-6">
      <div className="w-full max-w-xs rounded-3xl border-2 border-cyan-500/60 bg-slate-900 p-6 text-center shadow-2xl">
        ...
        <p className="mt-3 text-3xl font-extrabold tabular-nums text-cyan-200" dir="ltr">
          {state.percent}%
        </p>
        ...
        <p className="mt-2 text-xs text-slate-500">قد يستغرق بضع ثوانٍ حسب حجم التقرير</p>
      </div>
    </div>
  );
}
...
function OperationsReportScreen({
  ...
  /* نافذة سؤال طباعة المرفقات (نفس سلوك طباعة السند) */
  const [attachmentModalOpen, setAttachmentModalOpen] = useState(false);
  const [attachmentAskRows, setAttachmentAskRows] = useState<OperationItem[]>([]);
  ...
  /* طباعة التقرير التحليلي (سندات كاملة) مع/بدون المرفقات */
  const printAnalyticalVouchers = async (rowsToPrint: OperationItem[], includeAttachments: boolean) => {
    setPrintProgress(true, "جارٍ تجهيز السندات...", 10);
    await new Promise((resolve) => setTimeout(resolve, 50));

    // الصور تُقرأ من مخزنها المنفصل فقط عند طلب طباعتها
    /* بناء على دفعات: الصور تُقرأ وتُستهلك دفعة بدفعة بدل تحميلها كلها
       في الذاكرة مرة واحدة (10 آلاف مرفق = مئات الميجابايت وانهيار مؤكد). */
    const BUILD_BATCH = 100;
    const voucherPages: string[] = [];

    for (let i = 0; i < rowsToPrint.length; i += BUILD_BATCH) {
      const slice = rowsToPrint.slice(i, i + BUILD_BATCH);
      const resolved = includeAttachments ? await withResolvedImages(slice) : slice;
      ...
      for (const op of resolved) {
        ...
        voucherPages.push(
          buildVoucherInnerHtml({
            operation: op,
            settings,
            accountCode: accCode,
            cashboxCode: boxCode,
            includeAttachment: includeAttachments && Boolean(op.imageUrl),
          }),
        );
      }

      // تحرير الذاكرة المؤقتة للصور بعد استهلاكها
      if (includeAttachments) imageCache.clear();

      const pct = 10 + ((i + slice.length) / rowsToPrint.length) * 25;
      setPrintProgress(true, `جارٍ تجهيز السندات (${formatNumber(i + slice.length, 0)} من ${formatNumber(rowsToPrint.length, 0)})`, pct);
      await new Promise((resolve) => setTimeout(resolve, 0));
    }
  ...
  const printReport = async () => {
    setPrintProgress(true, "جارٍ حساب بيانات التقرير...", 5);
    await new Promise((resolve) => setTimeout(resolve, 50));
    ...
    if (reportMode === "تحليلي") {
      const withImages = printRows.filter((op) => op.imageUrl).length;

      // نفس سلوك طباعة السند: إن وُجدت مرفقات نسأل أولًا
      if (withImages > 0) {
        setPrintProgress(false);
        setAttachmentAskRows(printRows);
        setAttachmentCount(withImages);
        setAttachmentModalOpen(true);
        return;
      }

      await printAnalyticalVouchers(printRows, false);
      return;
    }
    ...
    printRows.forEach((op) => {
      ...
      pushToGroup(byCurrency, curr, op);
    });
    ...
    const blocks: PrintPageBlock[] = Array.from(byCurrency.entries()).map(([curr, currencyRows]) => {
      ...
      let totalDebit = 0;
      let totalCredit = 0;
      for (const x of currencyRows) {
        const value = getOperationAmounts(x).accountAmount;
        if (x.type === "صرف") totalDebit += value;
  ...
  return (
    <section className={`overflow-x-hidden space-y-4 rounded-3xl border p-4 sm:p-6 ${isDark ? "border-slate-700/70 bg-slate-900/60 text-slate-100" : "border-slate-300 bg-white/95 text-slate-900"}`}>
      ...
      {/* سؤال طباعة المرفقات — نفس نافذة طباعة السند */}
      {attachmentModalOpen ? (
        <ModalShell
        ...
        >
          <p className="text-sm">
            يحتوي هذا التقرير على {formatNumber(attachmentCount, 0)} سند بمرفقات
            {attachmentAskRows.length !== attachmentCount
              ? ` من أصل ${formatNumber(attachmentAskRows.length, 0)} سند`
              : ""}
            . هل تريد طباعة المرفقات أيضًا؟
          </p>
          {attachmentCount > MAX_ATTACHMENTS_PER_PRINT ? (
            <p className={`mt-2 rounded-xl border p-3 text-xs leading-relaxed ${isDark ? "border-rose-500/40 bg-rose-500/10 text-rose-200" : "border-rose-300 bg-rose-50 text-rose-800"}`}>
              ⛔ عدد المرفقات كبير جدًا ({formatNumber(attachmentCount, 0)} مرفق).
              الحد الأقصى للطباعة مع المرفقات هو {formatNumber(MAX_ATTACHMENTS_PER_PRINT, 0)} مرفق.
              <br />
              قلّل نطاق التاريخ أو اطبع بدون المرفقات.
            </p>
          ) : (
            <p className={`mt-2 text-xs ${isDark ? "text-amber-300" : "text-amber-700"}`}>
              ⚠️ طباعة المرفقات تزيد حجم التقرير وزمن التجهيز بشكل كبير
              {attachmentCount > 300 ? ` (قد تستغرق ${Math.ceil(attachmentCount / 150)} ثانية تقريبًا)` : ""}.
            </p>
          )}
          <div className="mt-4 flex flex-wrap gap-2">
            <button
              onClick={() => {
                const target = attachmentAskRows;
                setAttachmentModalOpen(false);
                void printAnalyticalVouchers(target, true);
              }}
              disabled={attachmentCount > MAX_ATTACHMENTS_PER_PRINT}
              className="rounded-xl bg-cyan-500 px-4 py-2 text-sm font-bold text-cyan-950 disabled:cursor-not-allowed disabled:bg-slate-600 disabled:text-slate-300"
            >
              نعم، مع المرفقات
            </button>
            <button
              onClick={() => {
                const target = attachmentAskRows;
                setAttachmentModalOpen(false);
                void printAnalyticalVouchers(target, false);
              }}
              className={`rounded-xl border px-4 py-2 text-sm ${isDark ? "border-slate-500 bg-slate-800" : "border-slate-300 bg-white"}`}
            >
              بدون المرفقات
            </button>
          </div>
...
function AccountsReportScreen({
  ...
  const build = () => {
    ...
    const analytical = Array.from(grouped.entries()).map(([key, entries]) => {
      ...
      // مرور واحد على السجلات بدل مرورين (filter + reduce مرتين)
      let debit = 0;
      let credit = 0;
      for (const e of entries) {
        const value = getOperationAmounts(e).accountAmount;
        if (e.type === "صرف") debit += value;
        else credit += value;
      }
  ...
  const printReport = async () => {
    ...
    built.aggregate.forEach((row) => pushToGroup(aggregateByCurrency, row.currency, row));
  ...
  const build = () => {
    ...
    return Array.from(grouped.entries()).map(([key, entries]) => {
      ...
      for (const entry of entries) {
        const value = getOperationAmounts(entry).cashboxAmount;
        if (entry.type === "قبض") debit += value;
        else credit += value;
      }
      const totalDebit = openingDebit + debit;
      const totalCredit = openingCredit + credit;
  ...
  const printReport = async () => {
    ...
    const blocks: PrintPageBlock[] = printRows.map((group) => {
      const tableRows = group.entries.map((entry) => {
        ...
        return `<tr>
              <td class="date-cell">${escapeHtml(parseDateOnly(entry.datetime))}</td>
              <td class="compact-cell">${escapeHtml(entry.type)}</td>
              <td class="compact-cell">${escapeHtml(entry.id)}</td>
              <td class="name-cell">${escapeHtml(entry.account)}</td>
              <td class="statement-cell">${escapeHtml(entry.statement ?? "—")}</td>
              <td class="amount-cell">${escapeHtml(debit ? formatNumber(debit) : "0")}</td>
              <td class="amount-cell">${escapeHtml(credit ? formatNumber(credit) : "0")}</td>
            </tr>`;
      });
      const openingRow = group.openingDebit || group.openingCredit
        ? `<tr><td></td><td></td><td></td><td></td><td class="statement-cell">رصيد افتتاحي</td><td class="amount-cell">${escapeHtml(group.openingDebit ? formatNumber(group.openingDebit) : "0")}</td><td class="amount-cell">${escapeHtml(group.openingCredit ? formatNumber(group.openingCredit) : "0")}</td></tr>`
        : "";
      ...
      return {
        ...
        theadHtml: `<thead><tr><th class="date-cell">التاريخ</th><th>النوع</th><th>رقم العملية</th><th>اسم الحساب</th><th>البيان</th><th class="amount-cell">مدين</th><th class="amount-cell">دائن</th></tr></thead>`,
        leadingRowsHtml: openingRow,
        rowsHtml: tableRows,
        footerHtml: `<tr><td colspan="5">الإجمالي</td><td class="amount-cell">${escapeHtml(formatNumber(group.debit))}</td><td class="amount-cell">${escapeHtml(formatNumber(group.credit))}</td></tr>`,
        tailHtml: `<div class="line">الرصيد: ${escapeHtml(formatBalanceWithNature(group.balance, group.currency, group.nature))}</div>`,
      };
    });
...
/* صورة تُحمَّل عند الحاجة فقط من مخزن الصور المنفصل */
function LazyImage({
  imageRef,
  alt,
  className,
  ...
  const [loaded, setLoaded] = useState<{ ref?: string; data?: string }>({});

  useEffect(() => {
    if (!isImageRef(imageRef)) return;
    let cancelled = false;
    void loadImage(imageRef).then((value) => {
      if (!cancelled) setLoaded({ ref: imageRef, data: value });
    });
    return () => {
      cancelled = true;
    };
  ...
  // الصور القديمة المضمّنة تُعرض مباشرة؛ المراجع تنتظر القراءة
  const src = isImageRef(imageRef)
    ? (loaded.ref === imageRef ? loaded.data : undefined)
    : imageRef;

  if (!src) {
    return (
      <div className={`${className ?? ""} grid place-items-center ${isDark ? "bg-slate-700/50" : "bg-slate-200"}`}>
        <span className={`text-xs ${isDark ? "text-slate-400" : "text-slate-500"}`}>جارٍ تحميل الصورة...</span>
      </div>
    );
  }
...
function ImageReportsScreen({
  ...
  const rows = useMemo(() => {
    ...
    return operations
      ...
      .sort((a, b) => (sortOrder === "newest" ? compareByDateTimeDesc(a, b) : compareByDateTimeAsc(a, b)));
  }, [operations, typeFilter, fromDate, toDate, fromAccount, toAccount, sortOrder, sectionFilter, query, availableAccountNames, accountByName]);
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {rows.map((row) => (
          <article key={`img-${row.type}-${row.id}-${row.datetime}`} className={`rounded-2xl border p-3 ${isDark ? "border-slate-700 bg-slate-800/70" : "border-slate-300 bg-slate-50"}`}>
            ...
            <LazyImage
              imageRef={row.imageUrl}
              alt={`صورة سند ${row.id}`}
              className="mt-2 h-44 w-full cursor-zoom-in rounded-xl object-cover"
              onOpen={(dataUrl) => setSelectedImage(dataUrl)}
              isDark={isDark}
            />
          </article>
        ))}
...
export default function HomePage() {
  ...
  /* حالة ترحيل الصور (تظهر مرة واحدة فقط بعد التحديث) */
  const [backgroundLoading, setBackgroundLoading] = useState(false);
  const pendingFullLoadRef = useRef(false);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (hasIndexedDb()) {
          ...
          /* ترحيل الصور القديمة إلى المخزن المنفصل — مرة واحدة فقط.
             يعمل بصمت ولا يؤثر على البيانات إن فشل. */
          try {
            const alreadyDone = await getStoreValue<boolean>(META_STORE, IMAGE_MIGRATION_KEY);
            if (!alreadyDone) {
              if (!cancelled) setMigrationBusy(true);
              const result = await migrateImagesToStore((done, total) => {
                if (!cancelled) setMigrationProgress({ done, total });
              });
              // نعلّم الترحيل منجزًا فقط إن لم تفشل أي صورة
              if (result.failed === 0) {
                await putStoreValue(META_STORE, IMAGE_MIGRATION_KEY, true);
              }
              if (!cancelled) setMigrationBusy(false);
            }
          } catch {
            if (!cancelled) setMigrationBusy(false);
          }

          /* تحميل تدريجي: نعرض أحدث صفحة فورًا ثم نُكمل الباقي في الخلفية.
             هذا يجعل الفتح شبه فوري مهما بلغ عدد السندات. */
          const totalPayments = await countOperations(PAYMENT_STORE);
          const totalReceipts = await countOperations(RECEIPT_STORE);
          const isLargeDataset = totalPayments + totalReceipts > FAST_START_THRESHOLD;

          let paymentRows: OperationItem[];
          let receiptRows: OperationItem[];

          if (isLargeDataset) {
            // الدفعة الأولى فقط — تكفي لعرض القوائم فورًا
            const [firstPayments, firstReceipts] = await Promise.all([
              readOperationsPage(PAYMENT_STORE, FAST_START_PAGE),
              readOperationsPage(RECEIPT_STORE, FAST_START_PAGE),
            ]);
            paymentRows = firstPayments.rows;
            receiptRows = firstReceipts.rows;
            pendingFullLoadRef.current = true;
          } else {
            paymentRows = await getAllOperations(PAYMENT_STORE);
            receiptRows = await getAllOperations(RECEIPT_STORE);
          }
        ...
        /* إكمال التحميل في الخلفية بعد ظهور الواجهة —
           التقارير تحتاج كل البيانات، لكن المستخدم لا ينتظرها. */
        if (pendingFullLoadRef.current) {
          pendingFullLoadRef.current = false;
          setBackgroundLoading(true);
          void (async () => {
            try {
              const [allPayments, allReceipts] = await Promise.all([
                getAllOperations(PAYMENT_STORE),
                getAllOperations(RECEIPT_STORE),
              ]);
              if (cancelled) return;
              setPaymentOperations(allPayments.sort((a, b) => b.datetime.localeCompare(a.datetime)));
              setReceiptOperations(allReceipts.sort((a, b) => b.datetime.localeCompare(a.datetime)));
            } catch {
              /* تبقى الصفحة الأولى معروضة */
            } finally {
    ...
    /* تنظيف ملفات العمل المؤقتة عند كل إقلاع (مثل التطبيقات الاحترافية):
       ملفات الطباعة والتصدير قد تتراكم إن أُغلق التطبيق أثناء العملية. */
    if (isAndroidApp()) {
      void NativeTools.clearWorkCache().catch(() => undefined);
    }
  ...
  const printNow = async (operationInput: OperationItem, includeAttachment: boolean) => {
    try {
      // جلب الصورة الفعلية من مخزنها المنفصل عند الحاجة فقط
      const operation = includeAttachment
        ? (await withResolvedImages([operationInput]))[0]
        : operationInput;
    ...
    const html = buildVoucherHtml({
      ...
      setPrintProgress(false); // لا يبقى المؤشر معلّقًا مهما حدث
    }
  };
  ...
  const createOperation = async (
    ...
    if (hasIndexedDb()) {
      try {
        // الصورة تُخزَّن في مخزنها المنفصل والسند يحمل مرجعها فقط
        created = await externalizeImage(created);
  ...
  const updateOperation = async (
    ...
    if (hasIndexedDb()) {
      try {
        // إن استُبدلت الصورة نحذف القديمة بعد نجاح الحفظ
        const previousRef = existing?.imageUrl;
        const externalized = await externalizeImage(updatedOperation);
        await putOperationRecord(storeName, externalized); // ← ننتظر التأكيد أولًا
        if (previousRef && previousRef !== externalized.imageUrl) void deleteImage(previousRef);
      } catch (error) {
        addToast(storageErrorMessage(error), "error");
        void refreshStorageStatus();
        return false;
  ...
  const deleteOperation = async (id: number, target: "payment" | "receipt") => {
    ...
    if (hasIndexedDb()) {
      try {
        const target_ = (target === "payment" ? paymentOperations : receiptOperations).find((op) => op.id === id);
        await deleteOperationRecord(storeName, id); // ← ننتظر التأكيد أولًا
        void deleteImage(target_?.imageUrl); // تنظيف الصورة المرتبطة
      } catch {
  ...
  const backupNow = async (passphrase?: string) => {
    ...
    setBackupBusy(true);
    try {
      setPrintProgress(true, "جارٍ تجهيز النسخة الاحتياطية...", 10);
      const payload = buildSnapshot();

      /* ═══ بناء النسخة على أجزاء (Streaming) ═══
         تجميع كل شيء في سلسلة نصية واحدة يتجاوز حد JavaScript
         (~512 ميجا حرف) مع آلاف الصور، فينهار التطبيق.
         نبني بدلًا من ذلك مصفوفة أجزاء ونمرّرها إلى Blob مباشرة. */
      const allOps = [...payload.paymentOperations, ...payload.receiptOperations];
      const imageCount = allOps.filter((op) => isImageRef(op.imageUrl)).length;

      const parts: string[] = [];
      const IMG_BATCH = 40;

      // 1) الرأس والبيانات العامة (بلا السندات)
      const { paymentOperations: _p, receiptOperations: _r, ...head } = payload;
      const headJson = JSON.stringify(head);
      parts.push(headJson.slice(0, -1)); // نزيل القوس الأخير لنُكمل يدويًا

      // 2) السندات على دفعات، مع إدراج الصور دفعة بدفعة
      const writeOps = async (key: string, ops: OperationItem[]) => {
        parts.push(`,"${key}":[`);
        for (let i = 0; i < ops.length; i += IMG_BATCH) {
          const slice = ops.slice(i, i + IMG_BATCH);
          const resolved = imageCount > 0 ? await withResolvedImages(slice) : slice;
          for (let j = 0; j < resolved.length; j += 1) {
            if (i + j > 0) parts.push(",");
            parts.push(JSON.stringify(resolved[j]));
          }
          if (imageCount > 0) imageCache.clear(); // تحرير الذاكرة فورًا
          const done = i + slice.length;
          setPrintProgress(true, `جارٍ تجهيز النسخة (${formatNumber(done, 0)} سند)`, 15 + (done / Math.max(1, ops.length)) * 30);
          await new Promise((resolve) => setTimeout(resolve, 0));
        }
        parts.push("]");
      };

      await writeOps("paymentOperations", payload.paymentOperations);
      await writeOps("receiptOperations", payload.receiptOperations);
      parts.push("}");

      setPrintProgress(true, "جارٍ إنشاء الملف...", 60);

      let blob: Blob;
      let filename = `proof-daftar-backup-${Date.now()}.json`;

      if (passphrase && passphrase.length > 0 && hasWebCrypto()) {
        /* التشفير يحتاج النص كاملًا. نتحقق أولًا أن الحجم آمن. */
        const approxSize = parts.reduce((sum, chunk) => sum + chunk.length, 0);
        if (approxSize > MAX_ENCRYPTED_BACKUP_CHARS) {
          setPrintProgress(false);
          addToast(
            `⛔ النسخة كبيرة جدًا للتشفير (${formatBytes(approxSize)}). احفظها بدون كلمة سر، أو قلّل عدد المرفقات`,
            "error",
          );
          return false;
        }
        const encrypted = await encryptWithPassphrase(parts.join(""), passphrase);
        blob = new Blob([JSON.stringify({ proofDaftar: true, appVersion: APP_VERSION, encrypted })], {
          type: "application/json",
        });
        filename = `proof-daftar-backup-${Date.now()}.pdbk.json`;
      } else {
        // Blob يقبل مصفوفة أجزاء — لا حاجة لدمجها في سلسلة واحدة
      ...
      if (isAndroidApp()) {
        /* الكتابة على دفعات إلى ملف مؤقت في الجهاز.
           نقرأ من الـ Blob على شرائح بدل تحميله كنص كامل في الذاكرة. */
        const CHUNK = 2 * 1024 * 1024;
        const totalChunks = Math.max(1, Math.ceil(blob.size / CHUNK));
        await NativeTools.beginStage();
        for (let offset = 0, step = 1; offset < blob.size; offset += CHUNK, step += 1) {
          const slice = blob.slice(offset, offset + CHUNK);
          await NativeTools.appendStage({ chunk: await slice.text() });
          setPrintProgress(true, "جارٍ حفظ النسخة الاحتياطية...", 65 + (step / totalChunks) * 30);
          await new Promise((resolve) => setTimeout(resolve, 0));
        }
  ...
  const applyRestore = async (text: string, passphrase?: string) => {
    try {
      ...
      try {
        if (hasIndexedDb()) {
          /* الصور المضمّنة في ملف النسخة تُنقل إلى مخزنها المنفصل،
             والسندات تحتفظ بمراجعها فقط. */
          const externalize = async (ops: OperationItem[], tag: string) => {
            const out: OperationItem[] = [];
            for (let i = 0; i < ops.length; i += 1) {
              const op = ops[i];
              if (op.imageUrl && !isImageRef(op.imageUrl)) {
                try {
                  const ref = `${IMAGE_REF_PREFIX}${tag}${op.id}_${Date.now()}_${i}`;
                  await putImageBlob(ref, op.imageUrl);
                  out.push({ ...op, imageUrl: ref });
                  continue;
                } catch {
                  /* عند الفشل تبقى الصورة مضمّنة — لا فقدان للبيانات */
                }
              }
              out.push(op);
          ...
          restoredSnapshot.paymentOperations = await externalize(restoredSnapshot.paymentOperations, "p");
          restoredSnapshot.receiptOperations = await externalize(restoredSnapshot.receiptOperations, "r");

          await saveSnapshotToIndexedDb(restoredSnapshot);
          await putStoreValue(META_STORE, IMAGE_MIGRATION_KEY, true);
        } else {
          localStorage.setItem(STORAGE_KEY, JSON.stringify(restoredSnapshot));
        }
      } catch (error) {
        addToast(storageErrorMessage(error), "error");
    ...
    >
      ...
      {printOptionModalOpen && printOperation ? (
        <ModalShell title="خيارات الطباعة" onClose={() => setPrintOptionModalOpen(false)} isDark={isDark}>
          ...
          <div className="mt-4 flex gap-2">
            <button onClick={() => { const op = printOperation; setPrintOptionModalOpen(false); setPrintOperation(null); void printNow(op, true); }} className="rounded-xl bg-cyan-500 px-4 py-2 text-sm font-bold text-cyan-950">نعم، مع المرفق</button>
            <button onClick={() => { const op = printOperation; setPrintOptionModalOpen(false); setPrintOperation(null); void printNow(op, false); }} className={`rounded-xl border px-4 py-2 text-sm ${isDark ? "border-slate-500 bg-slate-800" : "border-slate-300 bg-white"}`}>بدون المرفق</button>
          </div>
        </ModalShell>
      ) : null}
      ...
      {/* شاشة ترحيل الصور — تظهر مرة واحدة فقط بعد التحديث */}
      {migrationBusy ? (
        <div className="fixed inset-0 z-[96] flex items-center justify-center bg-slate-950/90 p-6">
          <div className="w-full max-w-sm rounded-3xl border-2 border-cyan-500/60 bg-slate-900 p-6 text-center shadow-2xl">
            <div className="mx-auto mb-4 size-12 animate-spin rounded-full border-4 border-slate-700 border-t-cyan-400" />
            <p className="text-lg font-bold text-cyan-300">جارٍ تحديث قاعدة البيانات</p>
            <p className="mt-2 text-sm text-slate-300">
              يتم تنظيم الصور المرفقة لتسريع التطبيق
            </p>
            {migrationProgress.total > 0 ? (
              <>
                <p className="mt-3 text-2xl font-extrabold tabular-nums text-cyan-200" dir="ltr">
                  {Math.round((migrationProgress.done / migrationProgress.total) * 100)}%
                </p>
                <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-slate-700">
                  <div
                    className="h-full rounded-full bg-cyan-400 transition-all duration-200"
            ...
            <p className="mt-4 rounded-xl bg-emerald-500/10 p-3 text-xs leading-relaxed text-emerald-300">
              ✅ جميع بياناتك وسنداتك محفوظة وسليمة.
              <br />
              تحدث هذه العملية مرة واحدة فقط — لا تغلق التطبيق.
            </p>
          </div>
        </div>
      ) : null}

      {/* مؤشر إكمال التحميل في الخلفية — لا يعيق الاستخدام */}
      {backgroundLoading ? (
        <div className="pointer-events-none fixed bottom-4 left-1/2 z-[70] -translate-x-1/2">
          <div className={`flex items-center gap-2 rounded-full border px-4 py-2 text-xs shadow-lg ${isDark ? "border-slate-600 bg-slate-800/95 text-slate-200" : "border-slate-300 bg-white/95 text-slate-700"}`}>
            <span className="size-3 animate-spin rounded-full border-2 border-slate-500 border-t-cyan-400" />
            جارٍ تحميل باقي السندات...
          </div>
        </div>
      ) : null}
